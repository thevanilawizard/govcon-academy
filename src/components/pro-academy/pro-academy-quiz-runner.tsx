...
import { isProQuizPassed } from "@/lib/pro-academy/progress";
import { PRO_LESSON_PASS_THRESHOLD } from "@/lib/pro-academy/types";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";

export function ProAcademyQuizRunner({
  title,
  subtitle,
  questions,
  farCitation,
  ...
  if (finished) {
    const passed = isProQuizPassed(score);
    return (
        ...
        <div className="p-4 rounded-lg border space-y-3">
          ...
          <p className="text-sm text-muted-foreground">
            ...
            {passed
              ...
              : `Need ${PRO_LESSON_PASS_THRESHOLD}% to pass.`}
          </p>
          {farCitation && (
            <p className="text-xs text-blue-800 bg-blue-50 p-2 rounded border border-blue-100">
              Regulatory basis: {farCitation}
            </p>
          )}
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      {revealed[q.id] && (
        <div className="p-3 rounded-lg bg-blue-50 border border-blue-100 text-sm text-blue-900 leading-relaxed space-y-2">
          <p>{q.explanation}</p>
          {farCitation && (
            <p className="text-xs font-medium text-blue-700">FAR reference: {farCitation}</p>
          )}
        </div>
      )}
